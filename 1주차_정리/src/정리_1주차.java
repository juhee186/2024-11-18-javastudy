 ...
 *        반복문
 *         =do~while : 무조건 1번 이상 수행  >조건을 나중에 처리
 *            do
 *            { 실행문장
 *            }while(조건문)   >>> 자바에서 사용하는 빈도는 적음
 *         =while  : 반복 횟수가 지정이 없는 경우, 무한루프
 *                   >> 데이터베이스, 파일 읽기
 *         =for    : 반복 횟수가 지정되어 있는 경우, 가장많이 사용되는 반복문
 *                   이중 for
 *                  ======> 알고리즘 : 정렬 / 빈도수 구하기 / 등수구하기
 *                  ======> 정보처리 실기, 코딩테스트
 *        반복제어문 : 반복문 제어
 *          =break : 반복문 종료
